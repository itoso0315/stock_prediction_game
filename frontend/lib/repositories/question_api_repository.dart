...
class QuestionApiRepository {
  QuestionApiRepository({required String baseUrl, http.Client? client})
  ...
  Future<List<Question>> getQuestions() async {
    ...
    try {
      ...
      throw QuestionApiException('Invalid questions response: $error');
    }
  }
  ...
  Future<Question> getResultQuestion(
    ...
    if (response.statusCode != 200) {
      throw QuestionApiException(
        'Failed to load result: HTTP ${response.statusCode}',
      );
    }
    try {
      final decoded = jsonDecode(utf8.decode(response.bodyBytes));
      ...
      throw QuestionApiException('Invalid result response: ${error.message}');
    } on TypeError catch (error) {
      throw QuestionApiException('Invalid result response: $error');
    }
  }
}
